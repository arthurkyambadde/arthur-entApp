...
var storeData = function getData() {
  const availableData = useMovieDataStore((state) => state.data);
...
var TrendingMovies = () => {
  ...
  const data2 = storeData2.filter((item) => item.isTrending);
...
var SelectedMovies = () => {
  ...
  const movieData = data2.filter((movie) => movie.category === "Movie");
...
var SelectedTVSeries = () => {
  const data2 = useMovieDataStore((state) => state.data);
  const serieData = data2.filter((serie) => serie.category === "TV Series");
 ...
 * LICENSE file in the root directory of this source tree.
 */
//# sourceMappingURL=/build/routes/index-JTSSFYCT.js.map

...
var SearchInput = import_react2.default.forwardRef(
  ({ children, ...props }, ref) => {
    const { searchFiled: searchFiled2, setSearchField } = props;
    const storeData = useMovieDataStore((state) => state.data);
    const filteredData = storeData.filter((movie) => {
      return movie.title.toLowerCase().includes(searchFiled2.toLowerCase());
    });
...
var AuthenticatedApplication = () => {
  ...
  const storeData = useMovieDataStore((state) => state.data);
  const filteredData = storeData.filter((movie) => {
    return movie.title.toLowerCase().includes(searchFiled.toLowerCase());
  });
  console.log("data", filteredData);
 ...
 * LICENSE file in the root directory of this source tree.
 */
//# sourceMappingURL=/build/routes/index-7GZ3HECR.js.map

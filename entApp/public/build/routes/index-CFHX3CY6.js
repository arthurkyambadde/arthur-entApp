...
var [searchField, setSearchField] = (0, import_react2.useState)("");
var storeData = useMovieDataStore((state) => state.data);
var filteredData = storeData.filter((movie) => {
...
var Thumbnail = (props) => {
  ...
  const [bookmark, setbookmark] = (0, import_react4.useState)(bookmarked);
...
var TextInput = import_react7.default.forwardRef(
  ({ children, ...props }, ref) => {
...
var TrendingMovies = () => {
  const storeData2 = useMovieDataStore((state) => state.data);
  const data = storeData2.filter((item) => item.isTrending);
...
var BookMarked = () => {
  const storeData2 = useMovieDataStore((state) => state.data);
  const bookmarkedMovieData = storeData2.filter(
    (item) => item.isBookmarked && item.category === "Movie"
  );
  const bookmarkedSerieData = storeData2.filter(
 ...
 * LICENSE file in the root directory of this source tree.
 */
//# sourceMappingURL=/build/routes/index-CFHX3CY6.js.map
